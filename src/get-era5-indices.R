...
getBUI <- function(res){
  ...
  for(i in id){
    ...
    for(j in 1:nrow(res_id)){
      ...
      bui <- buildup_index(row$dmc, row$dc)
      row <- mutate(row, bui = bui)
  ...
  bui_output <- do.call(rbind, iloop)
  return(bui_output)
}
...
getFWI <- function(res){
  ...
  for(i in id){
    ...
    for(j in 1:nrow(res_id)){
      ...
      fwi <- fire_weather_index(row$bui, row$isi)
      row <- mutate(row, fwi = fwi)
  ...
  fwi_output <- do.call(rbind, iloop)
  return(fwi_output)
}
